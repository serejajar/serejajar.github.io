...
function findPromo(value, promocodeArr) {
  const found = promocodeArr.find(item => item.promocode === value); // value содержит значение из поля ввода.

  return found // вернет объект с промокодом или undefined
}
