...
- Подскажите как реализовать модальное окно с данными, чтобы была возможность поделиться ссылкой.
Для каждого клиента добавляется ссылка с хешем с id, например

http://localhost:3000#123123
Если скопировать и перейти по этой ссылке, то откроется карточка клиента с его данными. Карточкой клиента может быть отдельная страница или модальное окно редактирования клиента.
