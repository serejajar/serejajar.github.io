Вы просто сохраняете изменения локально с помощью git add и git commit. Если хотите работать с другого компьютера, делаете git push, чтобы всё оказалось в репозитории. Закрывать что-либо специально не нужно — просто сохранённые коммиты остаются. Когда вернётесь, делаете git pull и продолжаете работу с актуальной версией файлов.
...
Я только одобряю мердж как это делается в рабочих проектах, а само слияние вы можете сделать самостоятельно после одобрения мерджа куратором.
