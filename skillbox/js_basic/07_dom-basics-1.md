...
# при повторном нажатии на кнопку в DOM добавляется еще одна таблица
Это не ошибка, но вам плюсик за то что вы улучшаете свою работу. Вы можете удалять предыдущую таблицу с помощью очищения элемента со списком. Для этого нужно создать listCardStudents один раз (в html или js) и дале присваивать innerHTML пустую строку:

let listCardStudents = ducument.querySelector('ваш css селектор')

listCardStudents.innerHTML = ''
