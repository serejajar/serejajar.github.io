...
Объяснение:
    ...
    join('') собирает обратно строку

Что проверяют:
работу со строками и массивами, базовые методы.

# JS задача с подвохом (знание Math.max)
Найти максимальное число в массиве
maxNumber([1, 5, 2, 9, 3]) // 9
...
оператор ... (spread) разворачивает массив в аргументы

Math.max находит максимальное число

Что проверяют:
spread-оператор, базовое понимание массивов.

# JS задача с подвохом (знание объектов)
const user1 = { name: 'Петр' }
const user2 = user1

user1.name = 'Федя'

console.log(user2.name) // Федя

# Почему не работает <button onclick="removePredmet()">Удалить</button>
См. пример в файле js_new/17-moduli.md

# Нет let / const
См. пример в файле javascript.md.

# ReactJS видео 4.4 Получение данных с сервера и 4.5 Отправка данных на сервер
Видео сильно переусложнено, тут можно сделать видео объсняющие все нюансы:
- Создание приложения на реакт без TS, react-query
- Добавление TS в приложение
- Добавление react-query в приложение
