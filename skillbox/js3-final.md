...
- Вы можете также получить все выбранные чекбоксы добавив в css селектор :checked. В этом случае вы можете не использовать if:
document.querySelectorAll(".custom-checkbox__field:checked").forEach((checkBox) => {
    filter.push(`${checkBox.value}`);
});
- Разделяйте компоненты и системные методы. Например, у вас есть несколько функция c fetch и при каждой загрузке страницы будет происходить несколько запросов за теми же данными. Тут лучше выполнить один раз запрос в main.js и передавать в каждую функцию массив данных в качестве аргумента.
window.addEventListener('DOMContentLoaded', () => {
    cons data = fetchData() // функция с запросом за данными​

    filterCount(data);
    /* остальной код */
})
- Разделяйте компоненты и системные методы. Например, у вас функция getDataCards c fetch.js в директории components, но там просто функция, которая делает запрос к данным. Это скорее хук или utils, но точно не компонент. Компонентом считает самостоятельная единица макета, которая обладает версткой, т.е. отображается в браузере, и логикой, т.е. у нее есть функционал на js. Компонентом можно считать корзину, слайдер, аккордион, элемент корзины, форма и тд.
...
###
Вопросы
# как фильтровать если несколь чекбоксов выбраны
Вам нужно вначале добавить обработчик фильтра для чекбокса.

checkbox.addEventListener("change",  () => {
  const filteredProducts = filterByType(products)
}))

Далее вам нужно реализовать функцию фильтрации. Тут основная сложность в том что нужно фильтровать по нескольким значениям, так как несколько чекбоксов быть отмечены, но это можно решить запросив все чекбоксы:
