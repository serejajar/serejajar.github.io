...
- Будьте осторожны когда используете addEventListener так как при его вызове не перезаписывается функция-обработчик клика, а добавляется новый. Например, каждый раз кода вы вызываете класс Basket  у вас происходит добавление еще одной функции-обработчика к кнопке basketBtnEl. Сейчас это не критично так как класс вызывается только один раз посто помните что это может привести к ошибкам.
