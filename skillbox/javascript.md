...
# Нет let / const
Нет let / const для переменной . Без let / const / var, JS в нестрогом режиме сохраняет значение в window:

window.curFilm = Object.values(item);

То есть переменная улетает в глобальную область видимости и становится доступна из любого места приложения, что означает что она может быть перезаписана кем угодно. Сейчас это не критично так как приложение состоит из одного скрипта, но в больших приложении это может привести к проблемам причину которых очень трудно обнаружить.

# Название objectVar это плохое название, так как название должно отражать то что хранит в себе переменная. Лучше:
const gift = randomGift(giftArr)
Старайтесь называть имена переменных так чтобы через время вам было понятно что делает этот код.
...
# какие элементы сайта сейчас принято делать на JS, а какие лучше оставлять в HTML?
Сейчас обычно структуру и базовую разметку делают в HTML, а JavaScript используют для интерактивности (обработки событий, динамического изменения DOM, загрузки данных и UI-логики.
...
# c isNaN проверять строку
С помощью isNaN проверять строку (releaseYear у вас срока) спорное решение так как на любое значение он вам выдаст false:

Для проверки releaseYear вам лучше привести эту переменную к числу

const releaseYear = Number(
  document.getElementById("releaseYear").value
);

Только после этого проверка с isNaN будет актуальна.
