По этому заданию все выполнено в соответствии с заданием и вам плюсик за корректное использование knex. Работа выполнена на отлично!

ДЗ принято.

Что можно улучшить
- Дублирование подключения knex. У вас в нескольких файлах создаётся отдельный экземпляр knex. Это создаёт несколько пулов подключений чего лучше избегать так как любая БД имеет лимит подключений, поэтому лучше создать db.js где будет инициализация knex:
...
И везде использовать этот файл

const knex = require('./db');

- Не хватает разбивки кода на небольшие части, использования express router, но это не критично для данного задания.
