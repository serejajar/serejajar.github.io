...
- duration таймеров не должны храниться в базе из-за большой нагрузки, или они каждый раз высчитываются основываясь от времени старта и текущего времени. Вот пример как подсчитывать progress
...
- Нет обработки ошибок в рутах
Если Mongo упадёт — сервер просто упадёт с unhandled rejection, поэтому лучше использовать try/catch
- В /timers/:id/stop можно сразу искать таймер по id, userId:

const timer = await db.collection("timers").findOne({ id, userId });
