...
Вы используете await внутри цикла поэтому пользователи обрабатываются последовательно, а это приведет к задержке в ответе последним пользователям. И чем больше количество пользователей, тем больше будет задержка. Например, при 200 пользователей может быть задержка в 1–4 секунд (каждый запрос в Mongo примерно 5–20ms).
