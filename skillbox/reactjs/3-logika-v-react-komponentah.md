...
- Вы мутируете объект прямо внутри массива:
item.title = title;
Это антипаттерн так как React ожидает иммутабельные обновления. Правильно вот так:
...
- map используется как forEach. map возвращает новый массив, а у вас он игнорируется
list.map((item) => {
Это плохая практика и на технических интервью при собеседовании выдает новичка в программировании. Лучше использовать методы там где нужно, forEach для перебора элементов массива, а map когда нужно создать новый массив.
